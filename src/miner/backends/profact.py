...
REPO_URL = "https://github.com/multimediaFor/ProFact.git"
MODEL_DIR = Path("models/profact")


def _ensure_repo() -> Path:
    repo_path = MODEL_DIR / "ProFact"
    if not repo_path.exists():
        logger.info("Cloning ProFact repo to %s", repo_path)
...
def _find_weights() -> Path | None:
    ...
    for candidate in [
        ...
        if candidate.exists() and any(
            list(candidate.glob("*.pth")) + list(candidate.glob("*.pt"))
        ):
            return candidate
    for candidate in [
        repo_path / "checkpoints",
...
class ProFactBackend(DetectionBackend):
    ...
    def _load_model(self):
        ...
        if weights_dir is None:
            raise FileNotFoundError(
                "ProFact weights not found. "
                "Run: python scripts/setup_models.py --install profact"
            )

        try:
            from models.profact import ProFact  # type: ignore[import-untyped]
            model = ProFact()
        except ImportError:
            try:
                from model import ProFact  # type: ignore[import-untyped]
                model = ProFact()
            except ImportError:
                logger.warning(
                    "ProFact model module not importable. "
                    "Ensure repo is set up: %s", repo_path
                )
                raise

        ckpt_files = list(weights_dir.glob("*.pth")) + list(weights_dir.glob("*.pt"))
        if ckpt_files:
    ...
    def detect(self, image: np.ndarray) -> tuple[bool, float, np.ndarray | None, ForgeryMethod | None]:
        ...
        # ProFact uses coarse-to-fine; take the final (finest) prediction
        if isinstance(output, (tuple, list)):
            pred = output[-1]
        elif isinstance(output, dict):
            pred = output.get("fine", output.get("mask", output.get("pred", list(output.values())[-1])))
        else:
            pred = output
