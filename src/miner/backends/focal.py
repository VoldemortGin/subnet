...
REPO_URL = "https://github.com/HighwayWu/FOCAL.git"
MODEL_DIR = Path("models/focal")
WEIGHTS_FILENAME = "focal_best.pth"


def _ensure_repo() -> Path:
    repo_path = MODEL_DIR / "FOCAL"
    if not repo_path.exists():
        logger.info("Cloning FOCAL repo to %s", repo_path)
...
class FOCALBackend(DetectionBackend):
    ...
    def _load_model(self):
        ...
        if weights_path is None:
            raise FileNotFoundError(
                "FOCAL weights not found. "
                "Run: python scripts/setup_models.py --install focal"
            )

        try:
            from model import FOCAL as FOCALModel  # type: ignore[import-untyped]
            model = FOCALModel()
        except ImportError:
            logger.warning(
                "FOCAL model module not importable. "
    ...
    def detect(self, image: np.ndarray) -> tuple[bool, float, np.ndarray | None, ForgeryMethod | None]:
        ...
        if isinstance(output, (tuple, list)):
            ...
            pred = output.get("mask", output.get("seg", list(output.values())[-1]))
        else:
            pred = output
