...
REPO_URL = "https://github.com/proteus1991/PSCC-Net.git"
MODEL_DIR = Path("models/psccnet")
WEIGHTS_FILENAME = "HRNet_checkpoint.pth"


def _ensure_repo() -> Path:
    repo_path = MODEL_DIR / "PSCC-Net"
    if not repo_path.exists():
        logger.info("Cloning PSCC-Net repo to %s", repo_path)
...
def _find_weights() -> Path | None:
    ...
    if repo_path is None:
        return None
    for candidate in [
        repo_path / "checkpoints" / WEIGHTS_FILENAME,
...
class PSCCNetBackend(DetectionBackend):
    ...
    def _load_model(self):
        ...
        if weights_path is None:
            raise FileNotFoundError(
                "PSCC-Net weights not found. "
                "Run: python scripts/setup_models.py --install psccnet"
            )

        try:
            from networks.PSCC import PSCCNet  # type: ignore[import-untyped]
            model = PSCCNet()
        except ImportError:
            logger.warning(
                "PSCC-Net model module not importable. "
    ...
    def detect(self, image: np.ndarray) -> tuple[bool, float, np.ndarray | None, ForgeryMethod | None]:
        ...
        if isinstance(output, (tuple, list)):
            ...
            seg_out = output.get("mask", output.get("seg", None))
        else:
            cls_out = None
            seg_out = output

        if cls_out is not None and isinstance(cls_out, torch.Tensor):
            prob = torch.sigmoid(cls_out).squeeze().cpu().numpy()
            confidence = float(prob) if prob.ndim == 0 else float(prob.max())
        else:
            confidence = 0.0
