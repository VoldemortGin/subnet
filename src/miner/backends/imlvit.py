...
REPO_URL = "https://github.com/SunnyHaze/IML-ViT.git"
MODEL_DIR = Path("models/imlvit")
WEIGHTS_FILENAME = "IML_ViT_CASIAv2.pth"


def _ensure_repo() -> Path:
    repo_path = MODEL_DIR / "IML-ViT"
    if not repo_path.exists():
        logger.info("Cloning IML-ViT repo to %s", repo_path)
...
class IMLViTBackend(DetectionBackend):
    ...
    def _load_model(self):
        ...
        if weights_path is None:
            raise FileNotFoundError(
                "IML-ViT weights not found. "
                "Run: python scripts/setup_models.py --install imlvit"
            )

        try:
            from models.IML_ViT import IML_ViT  # type: ignore[import-untyped]
            model = IML_ViT(
                image_size=1024,
                patch_size=16,
            ...
            logger.warning(
                "IML-ViT model module not importable. "
    ...
    def detect(self, image: np.ndarray) -> tuple[bool, float, np.ndarray | None, ForgeryMethod | None]:
        ...
        # IML-ViT may return (mask_pred, edge_pred) or a dict
        if isinstance(output, (tuple, list)):
            pred = output[0]
        elif isinstance(output, dict):
            pred = output.get("mask", output.get("pred", list(output.values())[0]))
        else:
            pred = output
